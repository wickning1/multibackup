...
class Tracker {
  ...
  toString () {
    ...
    const remaining = (this.goal - this.current) * elapsed / this.current
    return sprintf('%2.2f%% (%s / %s) (%s remaining) %s', progress, util.bytesToHuman(this.current), this.goalHuman, util.secondsToHuman(remaining), this.name)
  }
}
