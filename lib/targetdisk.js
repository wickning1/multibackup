...
class TargetDisk {
  ...
  async put(sourcefile) {
    ...
    await this.sftp.fastPut(source, dest, { step: function (transferred, chunk, totalsize) {
      console.log(transferred, totalsize, Math.round(10000.0 * transferred / totalsize)/100.0)
    }})
  }
  ...
  hasFile(file) {
    return this.files[file.interiorpath] ? true : false
  }

  hasValidFile(file) {
    const localfile = this.files[file.interiorpath]
    if (!localfile) return false
    if (localfile.size != file.size) return false
    return localfile.lastmod.isSameOrAfter(file.lastmod)
  }
