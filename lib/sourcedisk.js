...
class SourceDisk {
  ...
  async gather_dir(directorypath, files) {
    ...
    await Promise.all(dir.map(async function (item) {
      ...
      const interiorpath = path.resolve('/', path.relative(self.basepath, itempath))
      const stat = await fs.stat(itempath)
      if (stat.isFile()) {
        files[itempath] = new File(self, interiorpath, itempath.split(path.sep).join('/'), stat.mtime, stat.size)
