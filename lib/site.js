  ...
  async queuePut (file) {
    ...
    for (const disk of this.disks) {
      if (disk.hasFile(file)) {
        ...
        if (await disk.hasValidFile(file)) diskswithvalidfile.push(disk)
    ...
    while (invalidcount > 0) {
      ...
      // count does not change because we are replacing a file
      invalidcount--
    ...
    this.tracker.update('Determining backup status for each file... ' + (this.backupstatus++) + ' / ' + this.gathered + '.')
  }
